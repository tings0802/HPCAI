# Author: Yuting Shih
# Date: 2020/06/10 Wed.
# Desc: base image build from Untuntu 16.04
...
# OpenMPI 3.1.6, OpenMPI 3.0.6, and OpenMPI 4.0.3
RUN source ${SCRIPT_DIR}/ompi.sh

# Intel MPI 2020
COPY /storage/hpc/mpi/intel_sn ${WORK_DIR}
COPY /storage/hpc/mpi/parallel_studio_xe_2020_cluster_edition.tgz ${WORK_DIR}
RUN source ${SCRIPT_DIR}/impi.sh
...
CMD source /etc/profile.d/modules.sh && \
	module use ${MODULE_DIR} ${HPCX_MODULE} && \
	bash